...
|-------|------|----------|---------|-------------|
| `base_url` | `str` | No | http://localhost:5001 | Base URL of the Docling Serve instance. Do not include /v1 suffix - AsyncDoclingServiceClient adds it automatically. For backward compatibility, /v1 suffix will be stripped if present. |
...
| `mode` | `Literal[async, sync, auto]` | No | async | API mode: 'async' (use asynchronous submit/poll endpoints, recommended for both local and SaaS), 'sync' (use synchronous endpoints, fallback option), or 'auto' (detect server capabilities) |
