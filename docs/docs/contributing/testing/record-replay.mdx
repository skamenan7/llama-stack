...
- `ci-tests::config.yaml` — canonical config that CI and replay runs use. It already registers the Bedrock provider with a dummy API key, so recordings can be replayed without editing the file.
...
Prefer `ci-tests::config.yaml` for anything that interacts with the shared recordings, and switch to `bedrock-test.yaml` only when you need that minimal setup.
