...
```python
from ogx.testing.api_recorder import api_recording, APIRecordingMode
...
```bash
export OGX_TEST_INFERENCE_MODE=replay   # this is the default
export OGX_TEST_RECORDING_DIR=/path/to/recordings   # default is tests/integration/recordings
...
OGX_TEST_INFERENCE_MODE=record pytest test_failing.py
```

**Missing recordings:** New test or changed parameters
```bash
# Record the missing interaction
OGX_TEST_INFERENCE_MODE=record pytest test_new.py
```
