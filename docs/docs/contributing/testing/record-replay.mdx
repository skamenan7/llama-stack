...
### Stack Configuration Choices

- `ci-tests::run.yaml` — canonical config that CI and replay runs use. It already registers the Bedrock provider with a dummy API key, so recordings can be replayed without editing the file.
- `bedrock-test.yaml` — lightweight config at the repo root that only wires up Bedrock. Use this when you specifically want to run a standalone stack server for debugging.

Prefer `ci-tests::run.yaml` for anything that interacts with the shared recordings, and switch to `bedrock-test.yaml` only when you need that minimal setup.

### Recording Bedrock Tests

Record the three test functions (which generate 4 parametrized test cases):
