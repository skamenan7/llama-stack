...
> **_NOTE:_** Here's an example Pull Request of the [Milvus Vector Database Provider](https://github.com/ogx-ai/ogx/pull/1467).
...
4. **Add Tests**: Create unit tests and integration tests for your provider in the `tests/` directory.
   ...
   - Integration Tests
     - Integration tests are located in [tests/integration](https://github.com/ogx-ai/ogx/tree/main/tests/integration). These tests use the python client-SDK APIs (from the `ogx_client` package) to test functionality.
   ...
   - Updating the pyproject.yml
     - If you are adding tests for the `inline` provider or the `remote` provider you will have to update the `dev` group, which is used in the GitHub CI for unit and integration tests.
       - `uv add new_pip_package --group dev`
5. **Update Documentation**: Please update the documentation for end users
    - Generate the provider documentation by running [./scripts/provider_codegen.py](https://github.com/ogx-ai/ogx/blob/main/scripts/provider_codegen.py).
