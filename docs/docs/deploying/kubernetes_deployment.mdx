...
**Note:** For a complete list of supported distributions, see [distributions.json](https://github.com/ogx-ai/ogx-k8s-operator/blob/main/distributions.json) in the operator repository. Pre-built container images are available on [Docker Hub](https://hub.docker.com/u/ogxai) (e.g., `ogxai/distribution-starter`, `ogxai/distribution-postgres-demo`). To use a custom or non-supported distribution, set the `server.distribution.image` field with your container image instead of `server.distribution.name`.
