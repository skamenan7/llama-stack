...
Optionally moderate input and generated text within the Responses API. Configure an
external OpenAI-compatible `moderation_endpoint` on the builtin responses provider,
then enable it per request with `guardrails: true`. Authentication headers, when
needed, are configured on the server through `moderation_headers`.

Safety and Shields are no longer separate APIs or registered provider categories.
See [Configuring Responses Guardrails](/docs/distributions/configuration#configuring-responses-guardrails)
...
For detailed setup instructions, see our [Getting Started Guide](/docs/getting_started/quickstart).
