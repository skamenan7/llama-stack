...
**Use `distribution-builtin-gpu` if you:**
- Have access to GPU hardware
- Want maximum performance and control
- Need to run inference locally

```bash
docker pull llamastack/distribution-builtin-gpu
```

**Guides:** [Builtin GPU Guide](self_hosted_distro/builtin-gpu)
...
self_hosted_distro/builtin-gpu
```
