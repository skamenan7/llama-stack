...
No pre-built UI image is published to a container registry. Build it locally from the
`Containerfile` at `src/ogx_ui/Containerfile`, which installs the [`ogx-ui`](https://www.npmjs.com/package/ogx-ui)
npm package:

```bash
docker build -t ogx-ui -f src/ogx_ui/Containerfile .
