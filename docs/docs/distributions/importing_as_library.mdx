...
If you are planning to use an external service for Inference (even Ollama or vLLM counts as external), it is often easier to use OGX as a library.
...
For inference providers, pass per-request credentials keyed by the provider id, e.g. `{"<provider_id>_api_key": "..."}`. Keying by the provider id (rather than the backend type) lets you configure multiple providers of the same type and target each by id without the client needing to know the underlying backend. The server maps this key to the provider's internal credential field on each request.
...
```python
client = OGXAsLibraryClient(config_path)
```
