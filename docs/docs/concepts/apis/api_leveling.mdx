...
- Protocol classes: `Inference`, `Responses`, `VectorIO`, `Files`, `Batches`, etc. — the abstract interfaces a provider implements. Responses guardrails call a configured moderation endpoint; there is no separate `Safety` protocol.
...
Shared resource/value types (`Model`, `ToolGroup`, `VectorStore`) live canonically in `ogx_api.types` and are re-exported from `ogx_api.provider` for convenience — these are the only symbols intentionally present in both namespaces.
