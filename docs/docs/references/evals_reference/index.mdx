...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/meta-llama/ogx/blob/main/docs/notebooks/Llama_Stack_Benchmark_Evals.ipynb)
...
#### 1.1 Running MMMU
- We will use a pre-processed MMMU dataset from [ogx/mmmu](https://huggingface.co/datasets/ogx/mmmu). The preprocessing code is shown in this [GitHub Gist](https://gist.github.com/yanxi0830/118e9c560227d27132a7fd10e2c92840). The dataset is obtained by transforming the original [MMMU/MMMU](https://huggingface.co/datasets/MMMU/MMMU) dataset into correct format by `inference/chat-completion` API.

```python
import datasets

ds = datasets.load_dataset(path="ogx/mmmu", name="Agriculture", split="dev")
...
- We will use a pre-processed SimpleQA dataset from [ogx/evals](https://huggingface.co/datasets/ogx/evals/viewer/evals__simpleqa) which is obtained by transforming the input query into correct format accepted by `inference/chat-completion` API.
...
_ = client.datasets.register(
    ...
    source={
        "type": "uri",
        "uri": "huggingface://datasets/ogx/simpleqa?split=train",
    },
...
The following examples give the quick steps to start running evaluations using the ogx-client CLI.
...
```bash
ogx-client eval run_scoring <scoring_fn_id_1> <scoring_fn_id_2> ... <scoring_fn_id_n>
...
1. `EvalCandidate` to run generation on:
   - `ModelCandidate`: The model will be used for generation through OGX /inference API.
   - `AgentCandidate`: The agentic system specified by AgentConfig will be used for generation through OGX  /agents API.
...
To create new open-benchmark in ogx, you need to combine the prompt template and the raw data into the `chat_completion_input` column in the evaluation dataset.
...
You need to write a script [example convert script](https://gist.github.com/yanxi0830/118e9c560227d27132a7fd10e2c92840) to convert the benchmark raw dataset to ogx format eval dataset and update the dataset to huggingface [example benchmark dataset](https://huggingface.co/datasets/ogx/mmmu)
...
```bash
ogx-client eval run-benchmark <new_benchmark_id> \
--model_id <model id to evaluate on> \
--output_dir <directory to store the evaluate results> \
```
