...
2. `connect`: Connect third-party tools to the running OGX server. Currently supports `opencode` and `codex`. See [OpenCode Integration](../../building_applications/opencode_integration) and [Codex CLI Integration](../../building_applications/codex_cli_integration) for details.
