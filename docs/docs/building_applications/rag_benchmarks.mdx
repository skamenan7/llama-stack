...
We evaluated OGX against OpenAI across four benchmark suites covering retrieval accuracy, end-to-end answer quality, and multi-turn conversational RAG. OGX was tested in two search modes: **vector** (pure semantic search) and **hybrid** (semantic + keyword search with reranking).
...
| Benchmark | Type | OpenAI (F1) | OGX vector (F1) | OGX hybrid (F1) |
|---|---|---|---|---|
