...
- **Open-source models plug in without code changes.** Gemma 4 31B-IT, served via vLLM, produced coherent answers with zero empty responses across all 1,203 Doc2Dial queries. Lower F1 reflects response verbosity (~2,500 chars avg vs ~95 char ground truths), not retrieval failure — the same retrieval pipeline powers all OGX runs.
...
Gemma 4 31B-IT was served via vLLM and connected to OGX as a `remote::openai` inference provider, using the same retrieval pipeline as the GPT-4.1 runs. The model produced coherent, relevant answers with zero empty responses across all 1,203 Doc2Dial queries. Lower F1/ROUGE-L scores vs GPT-4.1 are driven by response verbosity, not retrieval failure — the same retrieval pipeline powers all OGX runs regardless of generation model. This demonstrates OGX's model-swappable architecture: open-source models can be plugged in without any code changes.
...
- For GPT-4.1 runs, answer quality differences reflect retrieval quality differences, not generation differences. For the Gemma run, the generation model's verbosity is an additional factor.
