...
class TestBedrockInferenceAdapterAuthMode:
    ...
    def test_client_uses_sigv4_auth_when_no_bearer_token(self):
        """Client property should inject SigV4 auth when no bearer token configured."""
        from llama_stack.providers.remote.inference.bedrock.bedrock import BedrockInferenceAdapter
        from llama_stack.providers.remote.inference.bedrock.config import BedrockConfig
        ...
        with patch.object(adapter, "get_request_provider_data", return_value=None):
            # Patch at source location (import happens inside _get_sigv4_auth)
            with patch("llama_stack.providers.utils.bedrock.sigv4_auth.BedrockSigV4Auth") as mock_auth_cls:
                ...
                # The SigV4 auth handler replaces the Bearer header with SigV4 signature
                assert client.api_key == "<NOTUSED>"

    @pytest.mark.skipif(not HAS_BOTO3, reason="boto3 not installed")
    def test_sigv4_auth_caches_auth_handler(self):
        """SigV4 auth handler should be cached for boto3 session reuse."""
        from llama_stack.providers.remote.inference.bedrock.bedrock import BedrockInferenceAdapter
        from llama_stack.providers.remote.inference.bedrock.config import BedrockConfig
        ...
        with patch.object(adapter, "get_request_provider_data", return_value=None):
            ...
            with patch("llama_stack.providers.utils.bedrock.sigv4_auth.BedrockSigV4Auth") as mock_auth_cls:
                ...
                # First access
                _ = adapter._get_sigv4_auth()
                assert mock_auth_cls.call_count == 1

                # Second access - should reuse cached auth
                _ = adapter._get_sigv4_auth()
                assert mock_auth_cls.call_count == 1  # Still 1, not 2


class TestHasAwsCredentials:
    """Tests for has_aws_credentials() utility function."""

    def test_detects_static_credentials(self, monkeypatch):
        """Should detect AWS_ACCESS_KEY_ID + AWS_SECRET_ACCESS_KEY."""
        from llama_stack.providers.utils.bedrock.sigv4_auth import has_aws_credentials
    ...
    def test_detects_web_identity(self, monkeypatch):
        """Should detect AWS_ROLE_ARN + AWS_WEB_IDENTITY_TOKEN_FILE."""
        from llama_stack.providers.utils.bedrock.sigv4_auth import has_aws_credentials

        monkeypatch.delenv("AWS_ACCESS_KEY_ID", raising=False)
        monkeypatch.delenv("AWS_SECRET_ACCESS_KEY", raising=False)
        monkeypatch.setenv("AWS_ROLE_ARN", "arn:aws:iam::123456:role/test")
        monkeypatch.setenv("AWS_WEB_IDENTITY_TOKEN_FILE", "/var/run/secrets/token")

        assert has_aws_credentials() is True

    def test_detects_profile(self, monkeypatch):
        """Should detect AWS_PROFILE."""
        from llama_stack.providers.utils.bedrock.sigv4_auth import has_aws_credentials
    ...
    def test_returns_false_when_no_credentials(self, monkeypatch, tmp_path):
        """Should return False when no credentials are detected."""
        from llama_stack.providers.utils.bedrock.sigv4_auth import has_aws_credentials
        ...
        # Point HOME to a temp directory without .aws/credentials
...
class TestWebIdentityFederation:
    ...
    def test_irsa_environment_detected(self, monkeypatch, tmp_path):
        """
        IRSA environment variables should be detected by has_aws_credentials().

        In Kubernetes with IRSA, these environment variables are automatically
        injected into the pod by the mutating webhook:
        ...
        # Set IRSA environment variables (as injected by EKS/OpenShift)
        monkeypatch.setenv("AWS_ROLE_ARN", "arn:aws:iam::123456789012:role/llama-stack-role")
        monkeypatch.setenv("AWS_WEB_IDENTITY_TOKEN_FILE", "/var/run/secrets/eks.amazonaws.com/serviceaccount/token")

        assert has_aws_credentials() is True

    def test_github_actions_environment_detected(self, monkeypatch, tmp_path):
        """
        GitHub Actions OIDC environment should be detected by has_aws_credentials().

        When using aws-actions/configure-aws-credentials with OIDC, the action sets:
        - AWS_ROLE_ARN: The IAM role to assume
        - AWS_WEB_IDENTITY_TOKEN_FILE: Path to the GitHub OIDC token
        """
        from llama_stack.providers.utils.bedrock.sigv4_auth import has_aws_credentials
        ...
        # Set GitHub Actions OIDC environment variables
        monkeypatch.setenv("AWS_ROLE_ARN", "arn:aws:iam::851725220677:role/github-actions-bedrock-role")
        monkeypatch.setenv("AWS_WEB_IDENTITY_TOKEN_FILE", "/tmp/github-oidc-token")
